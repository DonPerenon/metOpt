# lp solver by enumerating basic feasible solutions (bfs) using numpy only.
# supports: maximize c^T x, constraints A x (<=,=,>=) b, x>=0.
# file format identical to lp_simplex.py.
...
def parse(path):
    ...
    for ln in lines[2:2+m]:
        *coeff,sign,val = ln.split()
        A.append(list(map(float,coeff))); sgn.append(sign); b.append(float(val))
    return n,np.array(A,float),np.array(b,float),np.array(c,float),sgn

def enumerate_bfs(n,A,b,sgn,c):
    m=len(A)
    # build equality candidates: rows of A (treated as equalities) and nonnegativity x_j=0
    eqA = [A[i] for i in range(m)]
    eqb = [b[i] for i in range(m)]
    # add nonnegativity constraints
    for j in range(n):
        e=np.zeros(n); e[j]=1.0
        eqA.append(e); eqb.append(0.0)
    eqA=np.vstack(eqA); eqb=np.array(eqb)
    best=None; best_x=None
    idxs = range(len(eqA))
    # iterate all n-sized subsets
    for combo in itertools.combinations(idxs, n):
      ...
      for i in range(m):
          lhs=A[i]@x
          if sgn[i]=='<=' and lhs > b[i]+1e-8: feas=False; break
          if sgn[i]=='>=' and lhs < b[i]-1e-8: feas=False; break
          if sgn[i]=='='  and abs(lhs-b[i])>1e-8: feas=False; break
      if not feas: 
          continue
      z = float(np.dot(c,x))
